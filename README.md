...
5. Navigate to `src/Apps/ATSSystem.Api` and run `dotnet run` to launch the back end (ASP.NET Core Web API)
...
* `Sqlite`: ATSSystem.Infrastructure.Sqlite
* `SqlServer`: ATSSystem.Infrastructure.SqlServer
* `Npgsql`: ATSSystem.Infrastructure.Npgsql

### Multiple databases migrations
To use `dotnet-ef` for your migrations please add the following flags to your command (values assume you are executing from repository root)

* `--project src/Common/ATSSystem.Infrastructure.{DbProvider}`
* `--startup-project src/Apps/ATSSystem.Api`

For example, to add a new migration from the root folder:

set `"DbProvider"` in **appsettings.json** of Api project to `Sqlite`:
`dotnet ef migrations add "CreateDb" --project src\Common\ATSSystem.Infrastructure.Sqlite --startup-project src\Apps\ATSSystem.Api`

`dotnet ef database update --project src\Common\ATSSystem.Infrastructure.Sqlite --startup-project src\Apps\WebApi`

set `"DbProvider"` in **appsettings.json** of Api project to `SqlServer`:
`dotnet ef migrations add "CreateDb" --project src\Common\ATSSystem.Infrastructure.SqlServe --startup-project src\Apps\ATSSystem.Api`

`dotnet ef database update --project src\Common\ATSSystem.Infrastructure.SqlServer --startup-project src\Apps\WebApi`

set `"DbProvider"` in **appsettings.json** of Api project to `Npgsql`:
`dotnet ef migrations add "CreateDb" --project src\Common\ATSSystem.Infrastructure.Npgsql --startup-project src\Apps\ATSSystem.Api`

`dotnet ef database update --project src\Common\ATSSystem.Infrastructure.Npgsql --startup-project src\Apps\WebApi`
...
```powershell
PS ATSSystem> docker-compose up
```
...
If you are having problems, please let us know by [raising a new issue](https://github.com/iayti/ATSSystem/issues/new/choose).
